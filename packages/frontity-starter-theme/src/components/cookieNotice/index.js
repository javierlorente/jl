...
const CookieNotice = ({state}) => {
    ...
    <section sx={{variant: "notice"}}>
        <Box pt="xxs" mb="m">Utilizamos cookies propias y de terceros con finalidad analítica y mostrarte publicidad relevante para tus intereses</Box>
        <Flex sx={{ justifyContent: "flex-end"}}>
            <Button onClick={() => setConsent(status.rejected)} variant="buttons.link">Rechazar</Button>
            <Button onClick={() => setConsent(status.accepted)} variant="primary.gradient">Aceptar</Button>
        </Flex>
    </section>
    );
};
