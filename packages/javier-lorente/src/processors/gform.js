...
const Form = ({ children, formId, ...props }) => {
  ...
  const onError = (errors) => {
    ...
    const messagesNormalized = messages
  ...
  const sendForm = () => {
    ...
    return fetch(
      `https://frontity.javierlorente.es/wp-json/gf/v2/forms/${formId}/submissions`,
      options
    ).then((e) => e.json());
  };
  ...
  return (
    <>
      <Global
        ...
        `}
      />
      <form ref={formRef} onSubmit={hanldeSubmit} {...props}>
        {children}
        {successMessage && (
          <div
            role="alert"
            style={{ paddingTop: "1.5rem", fontWeight: "bold" }}
